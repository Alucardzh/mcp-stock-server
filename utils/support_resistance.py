...
def calculate_support_resistance(
    prices: pd.Series, n_levels: int = 5, lookback_period: int = 60
) -> dict[str, list[float]]:
    """
    计算支撑位和压力位
    ...
    # 5. 对候选点进行聚类，避免太接近的水平
    def cluster_levels(
        candidates: list[dict], n_target: int, merge_threshold: float = 0.02
    ) -> list[float]:
        """聚类相近的价格水平"""
        if not candidates:
            return []
        ...
        for candidate in candidates:
            ...
            for cluster in clusters:
                ...
                if abs(price - cluster_price) / cluster_price < merge_threshold:
                    ...
                    cluster["price"] = (
                        cluster["price"] * cluster["strength"] + price * strength
                    ) / total_strength
            ...
            if not added_to_existing:
                clusters.append({"price": price, "strength": strength, "count": 1})
    ...
    support_levels = [
        s for s in support_levels if s < current_price * 1.01
    ]  # 允许1%的误差
    resistance_levels = [r for r in resistance_levels if r > current_price * 0.99]
    ...
    if lookback_period < len(prices):  # 如果回看周期小于数据长度
        ...
        price_trend = (
            "up" if prices.iloc[-1] > prices.iloc[-lookback_period] else "down"
        )
    else:  # 如果回看周期大于等于数据长度
    ...
    ):  # 遍历所有基础压力位，i 为索引，price 为价格
        level = SupportResistanceLevel(  # 创建压力位级别对象
            price=float(price),  # 压力位价格，转换为浮点数
            level=i + 1,  # 压力位级别（从 1 开始）
            distance_from_current=float(price)
            - current_price_float,  # 距离当前价格的距离（压力位价格减去当前价格）
        )
